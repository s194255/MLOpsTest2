...
def test_model():
    ...
    assert Y.shape == torch.Size([batch_size, 10]), "model's output did not have correct shape"
...
def test_error_on_wrong_dtype():
    ...
    with pytest.raises(ValueError, match='Excepts dtype torch.float32 but got {}'.format(x.dtype)):
        model(x)